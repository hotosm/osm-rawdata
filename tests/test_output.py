...
def test_json():
    ...
    poly = geojson.load(infile)
    qc = QueryConfig()
    result = qc.parseJson(f"{rootdir}/levels.json")
    sql = db.createSQL(qc, True)
    ...
    new = geojson.loads(json)
    item = ['bank', 'ferry_terminal', 'bus_station', 'fuel', 'kindergarten', 'school', 'college', 'university', 'place_of_worship', 'marketplace', 'clinic', 'hospital', 'police', 'fire_station']
